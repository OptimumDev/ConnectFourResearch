...
Данный [тест](https://github.com/OptimumDev/ConnectFourResearch/blob/master/ConnectFourResearch/Tests/DepthTimeTests.cs#L16) показывает среднее время, за которое алгоритм успевает полностью закончить обработку на глубине `d`. Игры проводились против каждой реализации для того, чтобы разнообразить дерево игры. Алгоритмам давалось 300 мс на ход без ограничения глубины.
